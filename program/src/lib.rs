...
pub struct AvatarData{// Find out how to use the clock on solana
    ...
    pub level: u8,
    pub xp: u32,
>>>>>>> 044a23f63014708416157156a4b240e7ea58b005:src/lib.rs
}
...
<<<<<<< HEAD:program/src/lib.rs
        &instruction::create_metadata_accounts(
            ...
            *mint_authority_info.key,
            ...
            "Gtree".to_string(),
            select_uri(index_uri % 12).to_string(),
            Some(creators),
            500,
            true,
            true,
...
<<<<<<< HEAD:program/src/lib.rs
    let (locktime_pda, locktime_pda_bump) =
        Pubkey::find_program_address(locktime_pda_seed, program_id);
...
<<<<<<< HEAD:program/src/lib.rs
    let locktime_account_data: LockTime =
        try_from_slice_unchecked(&locktime_pda_info.data.borrow())?;
=======
    let avatar_data_pda_seed: &[&[u8]; 3] = &[
        b"avatar_data_pda",
        &mint_account_info.key.to_bytes(),
