...
use metaplex_token_metadata::{id, instruction, state::Creator};
use solana_program::{
...
pub struct AvatarData {
    // Find out how to use the clock on solana
...
fn select_uri<'life>(ind: u32) -> &'life str {
    ...
    if new_ind == 689 {
        let avatars = [
            "https://arweave.net/zREGv-Dt7Pw_CqvumRBvrGGTNg8-9EE6kQa-zcIpouM",
    ...
    //Proffesional
        ...
        let avatars = [
            "https://arweave.net/MmZN9UVtvP4wtDsb9AvtzJvzK1SrcRztLpbh9eJ6kKQ",
    ...
    //Rookie
        ...
        let avatars = [
            "https://arweave.net/L7_O0J0tw4Wi95bqFV9PY72RxCQ-Chr0glA0UJywALE",
    ...
    //Amature
        ...
        let avatars = [
            "https://arweave.net/Vp0TmR-V9-KveOrPDTkSJZr1-sixgMTMpkX6md0b7_o",
    ...
    //Local
        ...
        let avatars = [
            "https://arweave.net/kpQ38aRBSdtbwdKn61v626YSBMLR0JGGMXkRWBip_dQ",
    ...
    //Casual
        ...
        let avatars = [
            "https://arweave.net/tch0HhWhGM_nasO1UhN0l2LcTfbxv6H4ErmaXtguGZQ",
    ...
    //NewB
        ...
        let avatars = [
            "https://arweave.net/dERd63m8IANJibpM34YisI8Gkch1AHzT31ZL7wFPIy8",
...
pub fn mint_nft(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    let (mint_authority_pda, mint_authority_bump) =
        Pubkey::find_program_address(&[b"cyrial_pda"], program_id);
    ...
    invoke_signed(
        &instruction::create_metadata_accounts(
            id(),
            *metadata_pda_info.key,
            *mint_account_info.key,
            *mint_authority_info.key,
            *payer_account_info.key,
            *mint_authority_info.key,
            "Gamestree Avatar".to_string(),
            "Gtree".to_string(),
            select_uri(index_uri).to_string(),
            Some(creators),
            500,
            true,
            true,
    ...
    let (avatar_data_pda, avatar_data_pda_bump) =
        Pubkey::find_program_address(avatar_data_pda_seed, program_id);
    ...
    msg!(
        "Locked for {:?} seconds",
        unlockable_date - current_timestamp
    );
    Ok(())
}
...
fn unlock_account(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    let avatar_pda_account_data: AvatarData =
        try_from_slice_unchecked(&avatar_data_pda_info.data.borrow())?;
    let (avatar_data_pda, _avatar_data_pda_bump) =
        Pubkey::find_program_address(avatar_data_pda_seed, program_id);
    if avatar_data_pda_info.key != &avatar_data_pda {
        Err(ProgramError::InvalidAccountData)?
    }
...
fn claim_xp(program_id: &Pubkey, accounts: &[AccountInfo], to_increase_by: u32) -> ProgramResult {
    ...
    if !payer_account_info.is_signer
        || payer_account_info.key
            != &Pubkey::from_str("2ASw3tjK5bSxQxFEMsM6J3DnBozNh7drVErSwc7AtzJv").unwrap()
    {
        return Err(ProgramError::InvalidAccountData);
    }
    ...
    let mut avatar_pda_account_data: AvatarData =
        try_from_slice_unchecked(&avatar_data_pda_info.data.borrow())?;
    let (avatar_data_pda, _avatar_data_pda_bump) =
        Pubkey::find_program_address(avatar_data_pda_seed, program_id);
    if avatar_data_pda_info.key != &avatar_data_pda {
    ...
    avatar_pda_account_data.level =
        (0.01 * (avatar_pda_account_data.xp as f32).powf(1.0 / 3.0)).floor() as u8;
    avatar_pda_account_data.serialize(&mut &mut avatar_data_pda_info.data.borrow_mut()[..])?;
    Ok(())
}
...
pub fn process_instructions(
    ...
    match instruction {
        InstructionEnum::MintNft => mint_nft(program_id, accounts),
        InstructionEnum::UnlockMint => unlock_account(program_id, accounts),

        InstructionEnum::ClaimXp { xp_increase } => claim_xp(program_id, accounts, xp_increase),
    }
}
